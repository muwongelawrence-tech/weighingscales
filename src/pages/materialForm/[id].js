...
const validationSchema = Yup.object({
    ...
    price: Yup.number().min(0).max(80000).required().label("Price"),
...
const rawMaterialForm = () => {
   ...
   _id = router.query.id;
     ...
     try {
      ...
      const update_material = { _id, ...values };

      //console.log(update_material);
      
       const { data } = await updateRawMaterial(update_material);
               ...
               { ({ handleChange ,handleSubmit, errors ,values ,touched }) => (
                  <>
                     <form  className = "grid grid-cols-1" onSubmit = { handleSubmit }>
                          ...
                          <h1 className = "text-lg font-semibold mb-2 text-left text-white">Edit material</h1>
                          ...
                          />
                           ...
                           <textarea  placeholder = "Description"
                             className = "focus:outline-none border-0 p-6 md:p-10 w-auto m-2 text-black rounded-md resize-none"
                              rows = "3" cols = "30"
